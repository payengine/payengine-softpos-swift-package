...
/// A unique key used to ensure idempotency of the transaction.
/// The SDK automatically generates this key for each initiated transaction.
/// Partners can use this key to query transaction status in case of uncertain or network-failure scenarios.
@property (nonatomic, copy) NSString * _Nullable idempotentKey;
...
/// True if the transaction outcome cannot be determined
@property (nonatomic, readonly) BOOL isInconclusive;
@end
...
SWIFT_CLASS("_TtC18PEDevicePaymentSDK16TTPPaymentResult")
@interface TTPPaymentResult : NSObject <PEPaymentResult>
@property (nonatomic) enum PETransactionType transactionType;
...
@property (nonatomic, copy) NSString * _Nullable internalTransactionId;
@property (nonatomic, strong) PaymentResultHostReport * _Nullable hostReport;
@property (nonatomic, strong) PECardReaderError * _Nullable error;
@property (nonatomic, readonly) BOOL isInconclusive;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
